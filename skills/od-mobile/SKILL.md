...
1. `od-mobile` CLI가 설치돼 있으면 그대로 사용한다:
   ```bash
   od-mobile up        # Wi-Fi/사설망: tailnet에 노출 (tailnet IP로 접속)
   od-mobile tunnel    # 셀룰러/어디서든: Cloudflare 공개 HTTPS 터널 (Basic 인증)
   od-mobile ip        # tailnet IP 접속 주소
   od-mobile doctor    # 환경 진단
   od-mobile off       # 모든 노출 해제
   ```
